...
class OrderedDecoder(tf.keras.Model):
    ...
    def __init__(
        ...
        self.tag_embeddings = embedding_layer.EmbeddingSharedWeights(
            params["part_of_speech_size"], params["hidden_size"])
    ...
    def call(
        ...
        embedded_inputs = self.word_embeddings(words, mode="embedding")
        ...
        if word_indicators is None:
            word_indicators = tf.ones_like(words)
        ...
        tag_logits = self.tag_embeddings(slotted_activations, mode="linear")
        ...
        word_logits = self.word_embeddings(
            tf.concat([slotted_activations, tag_embeddings], 1), mode="embedding")
