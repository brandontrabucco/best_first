...
if __name__ == "__main__":
    ...
    for iteration, batch in enumerate(data_loader()):
        ...
        with writer.as_default():
            tf.summary.scalar("Eval Pointer Accuracy", pointer_accuracy)
            tf.summary.scalar("Eval Tag Accuracy", tag_accuracy)
            tf.summary.scalar("Eval Word Accuracy", word_accuracy)
            tf.summary.scalar("Eval Pointer Loss", pointer_loss)
            tf.summary.scalar("Eval Tag Loss", tag_loss)
            tf.summary.scalar("Eval Word Loss", word_loss)
            tf.summary.scalar("Eval Total Loss", total_loss)

        with writer.as_default():
            tf.summary.scalar("Eval Images Per Second", args.batch_size / (
